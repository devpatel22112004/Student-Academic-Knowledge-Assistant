...
def create_embeddings(chunks):
    ...
    return embeddings, model  # RETURN BECAUSE QUERY ALSO WE ARE GOING TO USE THIS MODEL TO CONVERT QUERY INTO EMBEDDING
...
def build_search_index(embeddings):
    ...
    faiss.normalize_L2(emb)  # Iska matlab vector ki length 1 kar di jaati hai FOR INNER PRODUCT
    ...
    return index  # RETURN FOR AGAIN QUERY MEIN USE KARNE KE LIYE


# HELPER FUNCTION: Extract keywords for lexical matching
...
# HELPER FUNCTION: Calculate lexical overlap score between query and chunk
def lexical_overlap_score(query, chunk_text):
    """
    Score chunk by keyword overlap with query.
    """
    q_keywords = set(extract_keywords(query))  # IMPORTANT WORDS IN THE QUERY
    if not q_keywords:
        return 0.0
...
def find_relevant_chunks(question, index, chunks, model, num_results=5):
    ...
    faiss.normalize_L2(question_embedding)  # Normalize question embedding for cosine similarity search
    ...
    candidates = []  # RERANK CANDIDATES USING A HYBRID SCORE (70% DENSE + 30% LEXICAL) TO IMPROVE EXACT-TOPIC RETRIEVAL
    for rank, chunk_idx in enumerate(indices[0]):
        ...
        candidates.append((hybrid, chunk_idx))  # APPEND THE HYBRID SCORE AND CHUNK INDEX TO THE CANDIDATES LIST

    candidates.sort(key=lambda x: x[0], reverse=True)  # HIGHEST HYBRID SCORE FIRST
...
# STEP 7: Generate an extractive answer from relevant chunks
def generate_extractive_answer(question, relevant_chunks, max_sentences=3):
    ...
    for chunk in relevant_chunks:
        sentences = re.split(r"(?<=[.!?])\s+", chunk["text"])  # SPLIT CHUNK TEXT INTO SENTENCES BASED ON PUNCTUATION
        for sent in sentences:
            if len(sent.strip()) < 20:  # SKIP VERY SHORT SENTENCES
                continue
            words = set(re.findall(r"[a-zA-Z0-9]+", sent.lower()))  # EXTRACT WORDS FROM THE SENTENCE FOR KEYWORD OVERLAP CALCULATION
            overlap = len(keywords.intersection(words)) if keywords else 0  # CALCULATE KEYWORD OVERLAP SCORE FOR EACH SENTENCE
            sentence_candidates.append((overlap, sent.strip(), chunk["source"]))  # APPEND SCORE, TEXT, AND SOURCE

    sentence_candidates.sort(key=lambda x: x[0], reverse=True)  # SORT SENTENCE CANDIDATES BY KEYWORD OVERLAP SCORE
    selected = sentence_candidates[:max_sentences]  # SELECT TOP SENTENCES TO FORM THE FINAL ANSWER

    if not selected:
        return "I could not find an exact sentence-level answer in the indexed documents.", []

    answer = " ".join([item[1] for item in selected])
    sources = [item[2] for item in selected]  # FOR ANSWER MAKE WITH JOIN
    return answer, sources


# MAIN PROGRAM - Starts here when you run this file directly
def main():
    print("=" * 70)
